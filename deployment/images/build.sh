...
buildKubeedgeEdge()
{
    docker build . -f ${DOCKERFILE_DIR}/${KUBEEDGE_EDGE_DOCKERFLE} --build-arg downloadscript_path=${DOWNLOAD_KUBEEDGE_SCRIPT_PATH} arch=$(getArch "$(uname -m)") -t apulis/kubeedge-edge:1.0
}
